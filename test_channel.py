...
if mask_type == "attention_plus":
    attention = True
    addloss = True
network = Network(H, W, message_length, noise_layers, device, batch_size, lr, attention, addloss, with_diffusion)
...
for i, images in enumerate(test_dataloader):
    ...
    with torch.no_grad():
        ...
        if attention:
            if addloss:
                encoded_images, mask, original_features, encoded_features = network.encoder_decoder.module.encoder(images, messages)
            else:
            ...
            # print(encoded_features.max())
            # print(encoded_features.min())
            # print(original_features.max())
            # print(original_features.min())

            # encoder_loss = criterion_MSE(images, encoded_images)

            # test mask channels
            # mask = mask.squeeze(0)
            # channel_num = mask.shape[0]
            # # print(mask.min())
            # # print(mask.max())
            # mask_channels = torch.zeros(channel_num)
            # for channel_id in range(channel_num):
            #     mask_channels[channel_id] = torch.mean(mask[channel_id])
            # topk_channels = torch.topk(mask_channels, 64, largest=False, sorted=True)
            # print(topk_channels)
            # continue
            ...
            # print(mask.shape)
            residual_features = encoded_features - original_features
            residual_features = residual_features.squeeze(0)
            # tmp = residual_features[saved_channels]
            # print(tmp.max())
            # print(tmp.min())
            # print(torch.mean(abs(tmp)))
            # continue
            ...
            # image diff
            for channel_id in range(channel_num):
                diff[channel_id] = abs(torch.mean(residual_features[channel_id]))
            topk_channels = torch.topk(diff, 5, largest=True, sorted=True)
            print(topk_channels)
            ...
            # encoded_images1 = images + (encoded_images - images) * strength_factor
            # noised_images = network.encoder_decoder.module.noise([encoded_images1, images])
            # decoded_messages = network.encoder_decoder.module.decoder(noised_images)
            # psnr = kornia.losses.psnr_loss(encoded_images1.detach(), images, 2).item()
            # ssim = 1 - 2 * kornia.losses.ssim(encoded_images1.detach(), images, window_size=5, reduction="mean").item()
            ...
            original_features = network.encoder_decoder.module.encoder.image_pre_layer(images)
            original_features = network.encoder_decoder.module.encoder.image_first_layer(original_features)
            encoded_features = network.encoder_decoder.module.encoder.image_pre_layer(encoded_images)
            encoded_features = network.encoder_decoder.module.encoder.image_first_layer(encoded_features)
